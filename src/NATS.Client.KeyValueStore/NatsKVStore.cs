...
public class NatsKVStore
{
    ...
    public async IAsyncEnumerable<NatsKVEntry<T>> WatchAsync<T>(string key, INatsSerializer<T>? serializer = default, NatsKVWatchOpts? opts = default, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
    ...
    public async IAsyncEnumerable<NatsKVEntry<T>> HistoryAsync<T>(string key, INatsSerializer<T>? serializer = default, NatsKVWatchOpts? opts = default, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        try
        {
            await GetEntryAsync<T>(key, cancellationToken: cancellationToken);
    ...
    /// <typeparam name="T">Serialized value type</typeparam>
    /// <returns>An asynchronous enumerable which can be used in <c>await foreach</c> loops</returns>
    public IAsyncEnumerable<NatsKVEntry<T>> WatchAsync<T>(INatsSerializer<T>? serializer = default, NatsKVWatchOpts? opts = default, CancellationToken cancellationToken = default) =>
