...
public partial class NatsConnection
{
    ...
    public ValueTask PublishAsync<T>(string subject, T? data, NatsHeaders? headers = default, string? replyTo = default, INatsSerialize<T>? serializer = default, NatsPubOpts? opts = default, CancellationToken cancellationToken = default)
    {
        ...
        opts.InboxPrefix ??= InboxPrefix;
        return PublishInternalAsync(opts, serializer, data, headers, default, cancellationToken);
    }
    ...
    private ValueTask PublishInternalAsync<T>(NatsPubOpts opts, INatsSerialize<T> serializer, T? data, NatsHeaders? headers = default, Activity? activity = default, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            task = PublishInternalAsync(opts, payloadBuffer.WrittenMemory, headersBuffer?.WrittenMemory, cancellationToken);
        }
        catch (Exception ex)
        {
            if (createEnd == null)
    ...
    private ValueTask PublishInternalAsync(NatsPubOpts opts, ReadOnlyMemory<byte> data, ReadOnlyMemory<byte>? headers = null, CancellationToken cancellationToken = default)
    {
        return ConnectionState != NatsConnectionState.Open
            ? ConnectAndPublishAsync(opts, data, headers, cancellationToken)
            : CommandWriter.PublishAsync(opts.Subject, data, headers, opts.ReplyTo, cancellationToken);
    }

    private async ValueTask ConnectAndPublishAsync(NatsPubOpts opts, ReadOnlyMemory<byte> data, ReadOnlyMemory<byte>? headers = null, CancellationToken cancellationToken = default)
    {
        await ConnectAsync().AsTask().WaitAsync(cancellationToken).ConfigureAwait(false);
        await CommandWriter.PublishAsync(opts.Subject, data, headers, opts.ReplyTo, cancellationToken).ConfigureAwait(false);
    }
}
