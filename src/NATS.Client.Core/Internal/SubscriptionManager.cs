...
internal sealed class SubscriptionManager : INatsSubscriptionManager, IAsyncDisposable
{
    ...
    public SubscriptionManager(NatsConnection connection, string inboxPrefix)
    {
        ...
        _replyTaskFactory = new ReplyTaskFactory(_connection);
    }
    ...
    {
        return _replyTaskFactory.CreateReplyTask<TReply>(deserializer, requestTimeout);
    }
        ...
        lock (_gate)
        {
            if (_bySid.TryGetValue(sid, out var sidMetadata))
            {
                ...
                if (_connection.Opts.RequestReplyMode == NatsRequestReplyMode.Direct)
                {
                    ...
                    //
                    // once records are available use props.SubjectNumber > 0 &  props.IsDirectReply
                    if (InboxSid == sid && subject.Length < _connection.InboxPrefix.Length + 1 + 22 + 1 + 22)
                    {
                        var idString = subject.AsSpan().Slice(_connection.InboxPrefix.Length + 1)
#if NETSTANDARD2_0
                    .ToString()
#endif
                        ;
