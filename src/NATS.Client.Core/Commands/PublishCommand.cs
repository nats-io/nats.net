...
internal sealed class PublishCommand<T> : CommandBase<PublishCommand<T>>
{
    ...
    private T? _value;
    private INatsSerialize<T>? _serializer;
    ...
    public static PublishCommand<T> Create(ObjectPool pool, string subject, string? replyTo, NatsHeaders? headers, T? value, INatsSerialize<T> serializer, Action<Exception>? errorHandler, CancellationToken cancellationToken)
    {
...
internal sealed class AsyncPublishCommand<T> : AsyncCommandBase<AsyncPublishCommand<T>>
{
    ...
    public static AsyncPublishCommand<T> Create(ObjectPool pool, CancellationTimer timer, string subject, string? replyTo, NatsHeaders? headers, T? value, INatsSerialize<T> serializer)
    {
        if (!TryRent(pool, out var result))
        {
