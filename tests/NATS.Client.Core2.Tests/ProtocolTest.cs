...
public class ProtocolTest
{
    ...
    public async Task Reconnect_with_sub_and_additional_commands()
    {
        ...
        var sync = 0;
        await using var sub = new NatsSubReconnectTest(nats, new NatsSubscriptionProps(subject), cmdSubject, i => Interlocked.Exchange(ref sync, i));
    ...
    private sealed class NatsSubReconnectTest : NatsSubBase
    {
        ...
        internal override async ValueTask WriteReconnectCommandsAsync(CommandWriter commandWriter, NatsSubscriptionProps props)
        {
            ...
            for (var i = 0; i < 100; i++)
            {
                await commandWriter.PublishAsync(new NatsPublishProps($"{_cmdSubject}{i}"), default, default, default);
            }
        }
