...
public class ConsumerConsumeTest
{
    ...
    public async Task Consume_msgs_test()
    {
        ...
        var count = 0;
        await using var cc = await consumer.ConsumeInternalAsync<TestData>(deserializer: TestDataJsonSerializer<TestData>.Default, consumerOpts, cancellationToken: cts.Token);
    ...
    public async Task Consume_idle_heartbeat_test()
    {
        ...
        var consumer = await js.GetConsumerAsync("s1", "c1", cts.Token);
        var count = 0;
        var cc = await consumer.ConsumeInternalAsync<TestData>(deserializer: TestDataJsonSerializer<TestData>.Default, consumerOpts, cancellationToken: cts.Token);
        await foreach (var msg in cc.Msgs.ReadAllAsync(cts.Token))
        {
    ...
    public async Task Consume_stop_test()
    {
        ...
        var cc = await consumer.ConsumeInternalAsync<TestData>(deserializer: TestDataJsonSerializer<TestData>.Default, consumerOpts, cancellationToken: consumeStop.Token);
