...
public static class Utils
{
    public static ValueTask<INatsJSConsumer> CreateConsumerAsync(this NatsJSContext context, string stream, string consumer, CancellationToken cancellationToken = default)
        => context.CreateConsumerAsync(new ConsumerCreateRequest { StreamName = stream, Config = new ConsumerConfiguration { Name = consumer, DurableName = consumer, AckPolicy = ConsumerConfigurationAckPolicy.@explicit } }, cancellationToken);
