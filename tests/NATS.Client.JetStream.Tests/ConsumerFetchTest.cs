...
public class ConsumerFetchTest
{
    ...
    public async Task Fetch_test()
    {
        ...
        var count = 0;
        await using var fc =
            await consumer.FetchAsync<TestData>(serializer: TestDataJsonSerializer<TestData>.Default, new NatsJSFetchOpts { MaxMsgs = 10 }, cancellationToken: cts.Token);
    ...
    public async Task FetchNoWait_test()
    {
        ...
        var consumer = await js.GetConsumerAsync("s1", "c1", cts.Token);
        var count = 0;
        await foreach (var msg in consumer.FetchAllNoWaitAsync<TestData>(serializer: TestDataJsonSerializer<TestData>.Default, new NatsJSFetchOpts { MaxMsgs = 10 }, cancellationToken: cts.Token))
        {
            await msg.AckAsync(new AckOpts(WaitUntilSent: true), cts.Token);
            Assert.Equal(count, msg.Data!.Test);
            count++;
    ...
    public async Task Fetch_dispose_test()
    {
        ...
        var fc = await consumer.FetchAsync<TestData>(serializer: TestDataJsonSerializer<TestData>.Default, fetchOpts, cancellationToken: cts.Token);
